...
def _write_recovery_journal(firmware: Path, backup: Path, digest: str) -> Path:
    journal = system._journal_path(firmware.resolve())
    journal.write_text(
        json.dumps({"backup": str(backup), "stock_sha256": digest}),
        encoding="utf-8",
    )
    return journal
...
def test_recover_firmware_rejects_backup_hash_mismatch(
    ...
    backup = local_tmp_path / "stock.bin"
    firmware.write_bytes(b"patched")
    backup.write_bytes(b"authentic stock")
    journal = _write_recovery_journal(firmware, backup, "0" * 64)
...
def test_recover_firmware_restores_verified_backup(
    ...
    journal = _write_recovery_journal(firmware, backup, system.sha256_bytes(stock))
    monkeypatch.setattr(system, "require_linux", lambda: None)
    monkeypatch.setattr(system, "_exclusive_lock", _unlocked_recovery)
    monkeypatch.setattr(
        system,
...
def test_experimental_apply_restores_stock_when_driver_load_fails(
    ...
    with pytest.raises(ApplyError, match="simulated module load failure"):
        system.experimental_apply(
            device.bdf,
            firmware,
            profile_580_105,
